...
# Remove import of dashboard/views.py to prevent circular imports
...
def setup_routes(app):
    """
    Registers all zone-related routes with the FastHTML application.
    """
    # Zone list view
    app.route("/zones", methods=["GET"])(zone_list_view)

    # Add zone
    app.route("/zones/add", methods=["GET", "POST"])(zone_add_view)

    # Edit zone
    app.route("/zones/edit/{zone_id:int}", methods=["GET", "POST"])(zone_edit_view)

    # Delete zone
    app.route("/zones/delete/{zone_id:int}", methods=["GET", "POST"])(zone_delete_view)

    # View routes within a specific zone
    app.route("/zones/{zone_id:int}/routes", methods=["GET"])(zone_routes_view)

    logger.info("All Zone routes have been successfully registered.")